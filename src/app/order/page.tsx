...
export default function DogCakeOrderForm() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Այստեղ կարող ես ավելացնել fetch -> API route կամ Telegram ուղարկում
    // Քարտեզան: ներկայում պատուհանով ցույց ենք տալիս տվյալները նախօրոք
    const summary: Record<string, string> = {};
    form.forEach((v, k) => {
      summary[k] = typeof v === "string" ? v : (v as File).name ?? "";
    });

    // օրինակ պարզ ցուցադրություն
    alert(
      (lang === "hy" ? "Պատվերը ստացել ենք։ Տեսքը՝\n\n" : lang === "en" ? "Order received. Summary:\n\n" : "Заказ принят. Сводка:\n\n") +
    ...
    <div className={styles.pageWrapper}>
      <div className={styles.container}>
        <div className={styles.langRow}>
          <button
            className={`${styles.langBtn} ${lang === "hy" ? styles.langActive : ""}`}
            onClick={() => setLang("hy")}
            type="button"
          >
            🇦🇲 Հայ
            ...
            <section key={section.title} className={styles.section}>
              ...
              <div className={styles.grid}>
                {section.fields.map((f) => {
                  ...
                  const inputType =
                    f === "email" ? "email" : f === "deliveryDate" ? "date" : f === "deliveryTime" ? "time" : f === "photo" ? "file" : "text";
                  return (
                    <div key={f} className={styles.field}>
                      ...
                      {isTextarea ? (
                        ...
                        <>
                          <input id={f} name={f} type="file" accept="image/*" onChange={handleFileChange} className={styles.fileInput} />
                          {previewSrc && <img src={previewSrc} alt="preview" className={styles.preview} />}
                        </>
                      ) : (
                        <input id={f} name={f} type={inputType} className={styles.input} required={["fullName", "phone", "dogName", "cakeType", "cakeSize", "address"].includes(f)} />
                      )}
                    </div>
                  );
                })}
